# Feature generation and pre-processing of paper 1
# user: muhdnoor
# password: WRDSaccess_135@
# Execution complete in 19 hours
# ============================================================================
...
class Paper1:
    ...
    def sp500_generate(self):
        years = list(
            range(int(self.wrds_start_date[-4:]), int(self.wrds_end_date[-4:]) + 1))
        count = 1
        for year in years:
            print('Year: ' + str(year) + ' is processing ... ')
            # Annual

            # Query data from WRDS
            conn = wrds.Connection(wrds_username=self.wrds_username)
            if count != len(years):
                self.parm = {"start_date": '01/01/' +
                             str(year), "end_date": '12/31/'+str(year)}
            else:
                self.parm = {"start_date": '01/01/' +
                             str(year), "end_date": self.wrds_end_date}
            ...
            raw_annual['act_l1'] = raw_annual.groupby(['permno'])[
                'act'].shift(1)

            # Lagged variable 3: che_l1
            raw_annual['che_l1'] = raw_annual.groupby(['permno'])[
                'che'].shift(1)

            # Lagged variable 4: lct_l1
            raw_annual['lct_l1'] = raw_annual.groupby(['permno'])[
                'lct'].shift(1)

            # Lagged variable 5: txp_l1
            raw_annual['txp_l1'] = raw_annual.groupby(['permno'])[
                'txp'].shift(1)
            ...
            raw_annual['dlc_l1'] = raw_annual.groupby(['permno'])[
                'dlc'].shift(1)

            # Lagged variable 8: csho_l1
            raw_annual['csho_l1'] = raw_annual.groupby(['permno'])[
                'csho'].shift(1)

            # Lagged variable 9: invt_l1
            raw_annual['invt_l1'] = raw_annual.groupby(['permno'])[
                'invt'].shift(1)

            # Lagged variable 10: dvt_l1
            raw_annual['dvt_l1'] = raw_annual.groupby(['permno'])[
                'dvt'].shift(1)

            raw_annual['ceq'] = np.where(
                raw_annual['ceq'] == 0, np.nan, raw_annual['ceq'])

            # Lagged variable 11: ceq_l1
            raw_annual['ceq_l1'] = raw_annual.groupby(['permno'])[
                'ceq'].shift(1)

            # Lagged variable 12: capx_l1
            raw_annual['capx_l1'] = raw_annual.groupby(['permno'])[
                'capx'].shift(1)

            # Lagged variable 13: ppent_l1
            raw_annual['ppent_l1'] = raw_annual.groupby(['permno'])[
                'ppent'].shift(1)

            # Lagged variable 14: rect_l1
            raw_annual['rect_l1'] = raw_annual.groupby(['permno'])[
                'rect'].shift(1)

            # Lagged variable 15: aco_l1
            raw_annual['aco_l1'] = raw_annual.groupby(['permno'])[
                'aco'].shift(1)

            # Lagged variable 16: intan_l1
            raw_annual['intan_l1'] = raw_annual.groupby(['permno'])[
                'intan'].shift(1)
            ...
            raw_annual['lco_l1'] = raw_annual.groupby(['permno'])[
                'lco'].shift(1)
            ...
            raw_annual['emp_l1'] = raw_annual.groupby(['permno'])[
                'emp'].shift(1)

            # Lagged variable 22: ppegt_l1
            raw_annual['ppegt_l1'] = raw_annual.groupby(['permno'])[
                'ppegt'].shift(1)
            ...
            raw_annual['sale_l1'] = raw_annual.groupby(['permno'])[
                'sale'].shift(1)

            # Lagged variable 25: cogs_l1
            raw_annual['cogs_l1'] = raw_annual.groupby(['permno'])[
                'cogs'].shift(1)

            # Lagged variable 26: xsga_l1
            raw_annual['xsga_l1'] = raw_annual.groupby(['permno'])[
                'xsga'].shift(1)
            ...
            raw_annual['xrd_l1'] = raw_annual.groupby(['permno'])[
                'xrd'].shift(1)
            ...
            raw_annual['capx_l2'] = raw_annual.groupby(['permno'])[
                'capx'].shift(2)

            # Lagged variable 32: capx_l2
            raw_annual['dltt_l1'] = raw_annual.groupby(['permno'])[
                'dltt'].shift(1)

            # Feature 2: acc
            raw_annual["acc"] = (raw_annual["ib"] - raw_annual["oancf"]) / \
            ...
            raw_annual["acc"] = np.where((raw_annual['oancf'].isnull()), ((((raw_annual["act"]
                                                                           ...
                                                                           - (raw_annual["lct"] - raw_annual["lct_l1"]) - (
                                                                               raw_annual["dlc"] - raw_annual["dlc_l1"])
                                                                           - (raw_annual["txp"] - raw_annual["txp_l1"]) - raw_annual["dp"]) / ((raw_annual["at"] + raw_annual["at_l1"]) / 2)), raw_annual['acc'])
            ...
            # Feature 4: agr
            raw_annual['agr'] = (raw_annual['at_l1'] -
                                 raw_annual['at']) / raw_annual['at_l1']

            # preferred stock
            raw_annual['pref_stock'] = np.where(
                raw_annual['pstkrv'].isnull(), raw_annual['pstkl'], raw_annual['pstkrv'])
            raw_annual['pref_stock'] = np.where(
                raw_annual['pref_stock'].isnull(), raw_annual['pstk'], raw_annual['pref_stock'])
            raw_annual['pref_stock'] = np.where(
                raw_annual['pref_stock'].isnull(), 0, raw_annual['pref_stock'])
            raw_annual['txditc'] = raw_annual['txditc'].fillna(0)

            # book equity
            raw_annual['be'] = raw_annual['seq'] + \
                raw_annual['txditc'] - raw_annual['pref_stock']
            raw_annual['be'] = np.where(
                raw_annual['be'] > 0, raw_annual['be'], np.nan)

            # Intermediate variable 1: mve_f
            raw_annual["mve_f"] = raw_annual["csho"] * \
                abs(raw_annual["prcc_f"])

            # Market Equity
            raw_annual['me'] = raw_annual['mve_f']  # Compustat ME
            raw_annual['me'] = np.where(
                raw_annual['me'] == 0, np.nan, raw_annual['me'])

            # Feature 5: bm
            raw_annual["bm"] = raw_annual["be"] / raw_annual["me"]

            # Feature 6: bm_ia
            df_temp = raw_annual.groupby(
                ['datadate', 'sic2'], as_index=False)['bm'].mean()
            df_temp = df_temp.rename(columns={'bm': 'bm_ind'})
            raw_annual = pd.merge(raw_annual, df_temp,
                                  how='left', on=['datadate', 'sic2'])
            raw_annual['bm_ia'] = raw_annual['bm'] / raw_annual['bm_ind']

            # Feature 7: cashdebt
            raw_annual["cashdebt"] = (
                raw_annual["ib"] + raw_annual["dp"]) / ((raw_annual["lt"] + raw_annual["lt_l1"]) / 2)

            # Feature 8: cashpr
            raw_annual["cashpr"] = (
                raw_annual["mve_f"] + raw_annual["dltt"] - raw_annual["at"]) / raw_annual["che"]
            ...
            # Feature 10: cfp_ia
            df_temp = raw_annual.groupby(['datadate', 'sic2'], as_index=False)[
                'cfp'].mean()
            ...
            raw_annual['chato'] = (raw_annual['sale']/((raw_annual['at']+raw_annual['at_l1'])/2))-(
                raw_annual['sale_l1']/((raw_annual['at']+raw_annual['at_l2'])/2))
            ...
            raw_annual = pd.merge(raw_annual, df_temp,
                                  how='left', on=['datadate', 'sic2'])
            raw_annual['chatoia'] = raw_annual['chato'] / \
                raw_annual['chato_ind']

            # Feature 12: chcsho
            raw_annual["chcsho"] = (
                raw_annual["csho"] / raw_annual["csho_l1"]) - 1

            # Feature 28: hire
            raw_annual['hire'] = (raw_annual['emp'] -
                                  raw_annual['emp_l1']) / raw_annual['emp_l1']
            raw_annual['hire'] = np.where((raw_annual['emp'].isnull()) | (
                raw_annual['emp_l1'].isnull()), 0, raw_annual['hire'])
            ...
            raw_annual = pd.merge(raw_annual, df_temp,
                                  how='left', on=['datadate', 'sic2'])
            raw_annual['chempia'] = raw_annual['hire'] / \
                raw_annual['chemp_ind']

            # Feature 14: chinv
            raw_annual["chinv"] = (raw_annual["invt"] - raw_annual["invt_l1"]) / \
                ((raw_annual["at"] + raw_annual["at_l1"]) / 2)

            # Intermediate variable 3: chpm
            raw_annual["chpm"] = (raw_annual["ib"] / raw_annual["sale"]) - \
                (raw_annual["ib_l1"] / raw_annual["sale_l1"])
            ...
            choicelist = [raw_annual['dcpstk'] -
                          raw_annual['pstk'], raw_annual['dcpstk']]

            raw_annual['dc'] = np.select(condlist, choicelist, default=np.nan)

            raw_annual['dc'] = np.where(
                raw_annual['dc'].isnull(), raw_annual['dcvt'], raw_annual['dc'])

            # Feature 16: convind
            raw_annual["convind"] = 1

            raw_annual["convind"] = np.where((raw_annual['dc'].notna() & raw_annual['dc'] != 0) | (
                raw_annual['cshrc'].notna() & raw_annual['cshrc'] != 0), raw_annual["convind"], 0)
            ...
            # Feature 19: divi
            raw_annual["divi"] = 1
            raw_annual["divi"] = np.where((raw_annual["dvt"].notna() & raw_annual["dvt"] > 0) & (
                (raw_annual["dvt_l1"] == 0) | (raw_annual["dvt_l1"].isnull())), raw_annual["divi"], 0)

            # Feature 20: divo
            raw_annual["divo"] = 1
            raw_annual["divo"] = np.where(((raw_annual["dvt"].isnull()) & (raw_annual["dvt"] == 0)) & (
                ((raw_annual["dvt_l1"] > 0) & (raw_annual["dvt_l1"])).notna()), raw_annual["divo"], 0)

            # Feature 21: dy
            raw_annual["dy"] = raw_annual["dvt"] / raw_annual["mve_f"]

            # Feature 22: egr
            raw_annual["egr"] = (raw_annual["ceq"] -
                                 raw_annual["ceq_l1"]) / raw_annual["ceq_l1"]

            # Feature 23: ep
            raw_annual["ep"] = raw_annual["ib"] / raw_annual["mve_f"]

            # Feature 24: gma
            raw_annual["gma"] = (raw_annual["revt"] -
                                 raw_annual["cogs"]) / raw_annual["at_l1"]

            # Feature 25: grcapx
            raw_annual['grcapx'] = (
                raw_annual['capx'] - raw_annual['capx_l2']) / raw_annual['capx_l2']
            ...
            raw_annual['grltnoa'] = ((raw_annual['rect']+raw_annual['invt']+raw_annual['ppent']+raw_annual['aco']+raw_annual['intan'] +
                                     ...
                                     - (raw_annual['rect_l1']+raw_annual['invt_l1']+raw_annual['ppent_l1']+raw_annual['aco_l1']
                                        + raw_annual['intan_l1']+raw_annual['ao_l1'] -
                                        raw_annual['ap_l1'] -
                                        raw_annual['lco_l1']
                                        - raw_annual['lo_l1'])
            ...
            raw_annual = pd.merge(raw_annual, df_temp,
                                  how='left', on=['datadate', 'sic2'])
            raw_annual['herf'] = (
                raw_annual['sale']/raw_annual['indsale'])*(raw_annual['sale']/raw_annual['indsale'])
            df_temp = raw_annual.groupby(['datadate', 'sic2'], as_index=False)[
                'herf'].sum()
            ...
            # Feature 29: invest
            raw_annual['invest'] = ((raw_annual['ppegt'] - raw_annual['ppegt_l1']) + (
                raw_annual['invt'] - raw_annual['invt_l1'])) / raw_annual['at_l1']
            raw_annual['invest'] = np.where((raw_annual['ppegt'].isnull()), ((raw_annual['ppent'] - raw_annual['ppent_l1']) + (
                raw_annual['invt'] - raw_annual['invt_l1'])) / raw_annual['at_l1'], raw_annual['invest'])
            ...
            raw_annual['xsga0'] = np.where(
                raw_annual['xsga'].isnull(), 0, raw_annual['xsga'])

            # Intermediate variable 6: xint0
            raw_annual['xint0'] = np.where(
                raw_annual['xint'].isnull(), 0, raw_annual['xint'])

            # Feature 33: operprof
            raw_annual['operprof'] = (raw_annual['revt'] - raw_annual['cogs'] -
                                      raw_annual['xsga0'] - raw_annual['xint0']) / raw_annual['ceq_l1']
            ...
            if count != len(years):
                cpi_data = fred.get_series('CPIAUCSL', observation_start=str(year)+'-01-01',
                                           observation_end=str(year)+'-12-31')
            else:
                cpi_data = fred.get_series('CPIAUCSL', observation_start=str(year)+'-01-01',
                                           observation_end=self.fred_end_date)
            #cpi_data = pd.read_csv("paper1/cpi_data.csv")
            cpi_data = pd.DataFrame(
                {'DATE': cpi_data.index, 'CPIAUCSL': cpi_data.values})
            ...
            raw_annual['orgcap_1'] = (
                raw_annual['xsga'] / raw_annual['CPIAUCSL']) / (0.1 + 0.15)
            raw_annual['orgcap'] = raw_annual['orgcap_1'] / raw_annual["avgat"]

            # Intermediate variable 10: pchcapx
            raw_annual["pchcapx"] = (
                raw_annual["capx"] - raw_annual["capx_l1"]) / raw_annual["capx_l1"]
            ...
            raw_annual = pd.merge(raw_annual, df_temp,
                                  how='left', on=['datadate', 'sic2'])
            raw_annual['pchcapx_ia'] = raw_annual['pchcapx'] / \
                raw_annual['pchcapx_ind']

            # Feature 36: pchcurrat
            raw_annual['pchcurrat'] = ((raw_annual['act']/raw_annual['lct'])-(
                raw_annual['act_l1']/raw_annual['lct_l1'])) / (raw_annual['act_l1']/raw_annual['lct_l1'])

            # Feature 37: pchdepr
            raw_annual["pchdepr"] = ((raw_annual["dp"] / raw_annual["ppent"]) - (
                raw_annual["dp_l1"] / raw_annual["ppent_l1"])) / (raw_annual["dp_l1"] / raw_annual["ppent_l1"])

            # Feature 38: pchgm_pchsale
            raw_annual['pchgm_pchsale'] = (((raw_annual['sale'] - raw_annual['cogs']) - (raw_annual['sale_l1']-raw_annual['cogs_l1']))/(
                raw_annual['sale_l1']-raw_annual['cogs_l1'])) - ((raw_annual['sale'] - raw_annual['sale_l1'])/raw_annual['sale'])
            ...
            raw_annual['pchsale_pchinvt'] = ((raw_annual['sale'] - raw_annual['sale_l1'])/raw_annual['sale_l1']) - (
                (raw_annual['invt']-raw_annual['invt_l1'])/raw_annual['invt_l1'])

            # Feature 41: pchsale_pchrect
            raw_annual['pchsale_pchrect'] = ((raw_annual['sale']-raw_annual['sale_l1'])/raw_annual['sale_l1']) - (
                (raw_annual['rect']-raw_annual['rect_l1'])/raw_annual['rect_l1'])

            # Feature 42: pchsale_pchxsga
            raw_annual['pchsale_pchxsga'] = ((raw_annual['sale']-raw_annual['sale_l1'])/raw_annual['sale_l1']) - (
                (raw_annual['xsga']-raw_annual['xsga_l1'])/raw_annual['xsga_l1'])

            # Feature 43: pchsaleinv
            raw_annual['pchsaleinv'] = ((raw_annual['sale']/raw_annual['invt'])-(
                raw_annual['sale_l1']/raw_annual['invt_l1'])) / (raw_annual['sale_l1']/raw_annual['invt_l1'])
            ...
            raw_annual['pctacc'] = np.select(condlist, choicelist, default=(
                raw_annual['ib']-raw_annual['oancf'])/raw_annual['ib'].abs())

            # Feature 45: ps

            raw_annual['p_temp1'] = np.where(raw_annual['ni'] > 0, 1, 0)
            raw_annual['p_temp2'] = np.where(raw_annual['oancf'] > 0, 1, 0)
            raw_annual['p_temp3'] = np.where(
                raw_annual['ni']/raw_annual['at'] > raw_annual['ni_l1']/raw_annual['at_l1'], 1, 0)
            raw_annual['p_temp4'] = np.where(
                raw_annual['oancf'] > raw_annual['ni'], 1, 0)
            raw_annual['p_temp5'] = np.where(
                raw_annual['dltt']/raw_annual['at'] < raw_annual['dltt_l1']/raw_annual['at_l1'], 1, 0)
            raw_annual['p_temp6'] = np.where(
                raw_annual['act']/raw_annual['lct'] > raw_annual['act_l1']/raw_annual['lct_l1'], 1, 0)
            raw_annual['p_temp7'] = np.where((raw_annual['sale']-raw_annual['cogs']/raw_annual['sale']) > (
                raw_annual['sale_l1']-raw_annual['cogs_l1']/raw_annual['sale_l1']), 1, 0)
            raw_annual['p_temp8'] = np.where(
                raw_annual['sale']/raw_annual['at'] > raw_annual['sale_l1']/raw_annual['at_l1'], 1, 0)
            raw_annual['p_temp9'] = np.where(raw_annual['scstkc'] == 0, 1, 0)
            raw_annual['ps'] = raw_annual['p_temp1']+raw_annual['p_temp2']+raw_annual['p_temp3']+raw_annual['p_temp4'] + \
                raw_annual['p_temp5']+raw_annual['p_temp6']+raw_annual['p_temp7'] + \
                raw_annual['p_temp8'] + raw_annual['p_temp9']

            # Feature 46: quick
            raw_annual["quick"] = (raw_annual["act"] -
                                   raw_annual["invt"]) / raw_annual["lct"]

            # Feature 47: rd
            raw_annual['xrd/at_l1'] = raw_annual['xrd']/raw_annual['at_l1']
            raw_annual['xrd/at_l1_l1'] = raw_annual.groupby(['permno'])[
                'xrd/at_l1'].shift(1)
            raw_annual['rd'] = np.where(((raw_annual['xrd']/raw_annual['at']) - (
                raw_annual['xrd/at_l1_l1']))/raw_annual['xrd/at_l1_l1'] > 0.05, 1, 0)
            ...
            raw_annual['realestate'] = (
                raw_annual['fatb']+raw_annual['fatl'])/raw_annual['ppegt']
            raw_annual['realestate'] = np.where(raw_annual['ppegt'].isnull(
            ), (raw_annual['fatb']+raw_annual['fatl'])/raw_annual['ppent'], raw_annual['realestate'])

            # Feature 51: roic
            raw_annual['roic'] = (raw_annual['ebit'] - raw_annual['nopi']) / \
                (raw_annual['ceq'] + raw_annual['lt'] - raw_annual['che'])
            ...
            raw_annual["securedind"] = np.where(raw_annual["dm"].notna(
            ) & raw_annual["dm"] != 0, raw_annual["securedind"], 0)

            # Feature 57: sgr
            raw_annual["sgr"] = (raw_annual["sale"] /
                                 raw_annual["sale_l1"]) - 1
            ...
            # Feature 60: tang
            raw_annual["tang"] = (raw_annual["che"] + raw_annual["rect"] * 0.715 +
                                  raw_annual["invt"] * 0.547 + raw_annual["ppent"] * 0.535) / raw_annual["at"]

            # Intermediate variable 11: tr

            condlist = ((raw_annual["fyear"] <= 1978), ((raw_annual["fyear"] >= 1978) & (raw_annual["fyear"] <= 1986)), (
                raw_annual["fyear"] == 1987), ((raw_annual["fyear"] >= 1988) & (raw_annual["fyear"] <= 1992)), (raw_annual["fyear"] >= 1993))
            choicelist = (0.48, 0.46, 0.40, 0.34, 0.35)
            raw_annual['tr'] = np.select(condlist, choicelist)

            # Intermediate variable 12: tb_1

            condlist = ((raw_annual["txfo"].isnull()) | (raw_annual["txfed"].isnull())),  ((
                (raw_annual["txfo"] + raw_annual["txfed"] > 0) | (raw_annual["txt"] > raw_annual["txdi"])) & (raw_annual["ib"] <= 0))
            choicelist = (
                (((raw_annual["txt"] - raw_annual["txdi"]) / raw_annual["tr"]) / raw_annual["ib"]), 1)
            raw_annual['tb_1'] = np.select(condlist, choicelist, default=(
                (raw_annual["txfo"] + raw_annual["txfed"]) / raw_annual["tr"]) / raw_annual["ib"])
            ...
            raw_annual = raw_annual.drop_duplicates(
            ...
            raw_quarter = comp_quarter.merge(
                comp_crsp_link[["gvkey", "permno", "linkdt", "linkenddt"]], how="left", on=["gvkey"])
            raw_quarter["permno"] = raw_quarter["permno"].astype("Int64")

            raw_quarter["datadate"] = pd.to_datetime(raw_quarter["datadate"])
            raw_quarter = raw_quarter.sort_values(
                ["permno", "datadate"]).drop_duplicates()

            # Set link date bounds
            raw_quarter = raw_quarter[(raw_quarter['datadate'] >= raw_quarter['linkdt']) & (
                raw_quarter['datadate'] <= raw_quarter['linkenddt'])]
            ...
            # chtx
            raw_quarter["txtq_l4"] = raw_quarter.groupby(["permno"])[
                "txtq"].shift(4)
            raw_quarter["atq_l4"] = raw_quarter.groupby(["permno"])[
                "atq"].shift(4)
            raw_quarter["chtx"] = (
                raw_quarter["txtq"] - raw_quarter["txtq_l4"]) / raw_quarter['atq_l4']

            # roaq
            raw_quarter["atq_l1"] = raw_quarter.groupby(["permno"])[
                "atq"].shift(1)
            raw_quarter["roaq"] = raw_quarter["ibq"] / raw_quarter["atq_l1"]

            # roeq
            raw_quarter["pstk"] = np.where(
                raw_quarter["pstkrq"].isnull(), raw_quarter["pstkq"], raw_quarter["pstkrq"])
            condlist = [raw_quarter["seqq"].isnull(),
                        raw_quarter["seqq"].isnull() & (raw_quarter["ceqq"].isnull() | raw_quarter["pstk"].isnull())]
            choicelist = [raw_quarter["ceqq"] + raw_quarter["pstk"],
                          raw_quarter["atq"] - raw_quarter["ltq"]]
            raw_quarter["scal"] = np.select(
                condlist, choicelist, default=raw_quarter["seqq"])
            raw_quarter["scal_l1"] = raw_quarter.groupby(["permno"])[
                "scal"].shift(1)
            raw_quarter["roeq"] = raw_quarter["ibq"] / raw_quarter["scal_l1"]

            # rsup
            # raw_quarter["mveq"] = abs(raw_quarter["prccq"]) * raw_quarter["cshoq"]
            raw_quarter["saleq_l4"] = raw_quarter.groupby(["permno"])[
                "saleq"].shift(4)
            raw_quarter["rsup"] = (
                raw_quarter["saleq"] - raw_quarter["saleq_l4"]) / raw_quarter["mveq"]

            # cinvest
            for i in range(4):
                raw_quarter["ppentq_l" +
                            str(i+1)] = raw_quarter.groupby(["permno"])["ppentq"].shift(i+1)
                raw_quarter["saleq_l" +
                            str(i+1)] = raw_quarter.groupby(["permno"])["saleq"].shift(i+1)

            raw_quarter["cinv_1"] = (
                raw_quarter["ppentq_l1"] - raw_quarter["ppentq_l2"]) / raw_quarter["saleq_l1"]
            raw_quarter["cinv_2"] = (
                raw_quarter["ppentq_l2"] - raw_quarter["ppentq_l3"]) / raw_quarter["saleq_l2"]
            raw_quarter["cinv_3"] = (
                raw_quarter["ppentq_l3"] - raw_quarter["ppentq_l4"]) / raw_quarter["saleq_l3"]
            ...
            raw_quarter["cinv_4"] = (
                raw_quarter["ppentq_l1"] - raw_quarter["ppentq_l2"]) / 0.01
            raw_quarter["cinv_5"] = (
                raw_quarter["ppentq_l2"] - raw_quarter["ppentq_l3"]) / 0.01
            raw_quarter["cinv_6"] = (
                raw_quarter["ppentq_l3"] - raw_quarter["ppentq_l4"]) / 0.01
            ...
            raw_quarter = raw_quarter.drop(
                ["cinv_1", "cinv_2", "cinv_3", "cinv_4", "cinv_5", "cinv_6"], axis=1)

            # nincr
            temp_nincr = []
            for i in range(8):
                raw_quarter["ibq_l" +
                            str(i+1)] = raw_quarter.groupby(["permno"])["ibq"].shift(i+1)
                temp_nincr.append("ibq_l" + str(i+1))
                if i == 0:
                    raw_quarter["nincr_t" + str(i+1)] = np.where(
                        raw_quarter["ibq"] > raw_quarter["ibq_l1"], 1, 0)
                else:
                    raw_quarter["nincr_t" + str(i+1)] = np.where(
                        raw_quarter["ibq_l" + str(i)] > raw_quarter["ibq_l" + str(i+1)], 1, 0)
                temp_nincr.append("nincr_t" + str(i+1))

            raw_quarter["nincr"] = (raw_quarter["nincr_t1"]
                                    + (raw_quarter["nincr_t1"]
                                       * raw_quarter["nincr_t2"])
                                    + (raw_quarter["nincr_t1"] * raw_quarter["nincr_t2"] * raw_quarter["nincr_t3"])
                                    + (raw_quarter["nincr_t1"] * raw_quarter["nincr_t2"]
                                       * raw_quarter["nincr_t3"] * raw_quarter["nincr_t4"])
                                    + (raw_quarter["nincr_t1"] * raw_quarter["nincr_t2"] *
                                       raw_quarter["nincr_t3"] * raw_quarter["nincr_t4"] * raw_quarter["nincr_t5"])
                                    + (raw_quarter["nincr_t1"] * raw_quarter["nincr_t2"] * raw_quarter["nincr_t3"]
                                       * raw_quarter["nincr_t4"] * raw_quarter["nincr_t5"] * raw_quarter["nincr_t6"])
                                    + (raw_quarter["nincr_t1"] * raw_quarter["nincr_t2"] * raw_quarter["nincr_t3"] *
                                       raw_quarter["nincr_t4"] * raw_quarter["nincr_t5"] * raw_quarter["nincr_t6"] * raw_quarter["nincr_t7"])
            ...
            for i in range(16):
                raw_quarter["roaq_l" +
                            str(i)] = raw_quarter.groupby(["permno"])["roaq"].shift(i)
            ...
            # stdacc
            raw_quarter["actq_l1"] = raw_quarter.groupby(["permno"])[
                "actq"].shift(1)
            raw_quarter["cheq_l1"] = raw_quarter.groupby(["permno"])[
                "cheq"].shift(1)
            raw_quarter["lctq_l1"] = raw_quarter.groupby(["permno"])[
                "lctq"].shift(1)
            raw_quarter["dlcq_l1"] = raw_quarter.groupby(["permno"])[
                "dlcq"].shift(1)
            ...
            for i in range(16):
                raw_quarter["sacc_l" +
                            str(i)] = raw_quarter.groupby(["permno"])["sacc"].shift(i)
            ...
            # stdcf
            raw_quarter["scf"] = (raw_quarter["ibq"] /
                                  raw_quarter["saleq"]) - raw_quarter["sacc"]
            raw_quarter["scf"] = np.where(raw_quarter["saleq"] <= 0, (
                raw_quarter["ibq"] / 0.01) - raw_quarter["sacc"], raw_quarter["scf"])

            temp_stdcf = []
            for i in range(16):
                raw_quarter["scf_l" +
                            str(i)] = raw_quarter.groupby(["permno"])["scf"].shift(i)
            ...
            comp_annual1 = comp_annual1.merge(
                comp_crsp_link[["gvkey", "permno"]], how="left", on=["gvkey"])
            comp_annual1["permno"] = comp_annual1["permno"].astype("Int64")
            comp_annual1["datadate"] = pd.to_datetime(comp_annual1["datadate"])
            comp_annual1 = comp_annual1.sort_values(
                ["permno", "datadate"]).drop_duplicates()
            ...
            comp_annual1["at_l1"] = comp_annual1.groupby(["permno"])[
                "at"].shift(1)
            comp_annual1["roa"] = comp_annual1["ni"] / \
                ((comp_annual1["at"] + comp_annual1["at_l1"]) / 2)
            comp_annual1["cfroa"] = comp_annual1["oancf"] / \
                ((comp_annual1["at"] + comp_annual1["at_l1"]) / 2)
            ...
            comp_annual1["xrdint"] = comp_annual1["xrd"] / \
                ((comp_annual1["at"] + comp_annual1["at_l1"]) / 2)
            comp_annual1["capxint"] = comp_annual1["capx"] / \
                ((comp_annual1["at"] + comp_annual1["at_l1"]) / 2)
            comp_annual1["xadint"] = comp_annual1["xad"] / \
                ((comp_annual1["at"] + comp_annual1["at_l1"]) / 2)

            # Calculate annual industry averages
            df_temp = comp_annual1.groupby(["fyear", "sic2"], as_index=False)[
                ["roa", "cfroa", "xrdint", "capxint", "xadint"]].median()
            df_temp = df_temp.rename(columns={"roa": "md_roa", "cfroa": "md_cfroa", "xrdint": "md_xrdint",
                                              "capxint": "md_capxint", "xadint": "md_xadint"})
            comp_annual1 = pd.merge(
                comp_annual1, df_temp, how="left", on=["fyear", "sic2"])

            # Calculate annual values
            comp_annual1["m1"] = np.where(
                comp_annual1["roa"] > comp_annual1["md_roa"], 1, 0)
            comp_annual1["m2"] = np.where(
                comp_annual1["cfroa"] > comp_annual1["md_cfroa"], 1, 0)
            comp_annual1["m3"] = np.where(
                comp_annual1["oancf"] > comp_annual1["ni"], 1, 0)
            comp_annual1["m4"] = np.where(
                comp_annual1["xrdint"] > comp_annual1["md_xrdint"], 1, 0)
            comp_annual1["m5"] = np.where(
                comp_annual1["capxint"] > comp_annual1["md_capxint"], 1, 0)
            comp_annual1["m6"] = np.where(
                comp_annual1["xadint"] > comp_annual1["md_xadint"], 1, 0)

            # Merge to raw_quarter
            merge_cols = ["permno", "fyear", "m1",
                          "m2", "m3", "m4", "m5", "m6"]
            ...
            for i in range(15):
                raw_quarter["rsup_l" +
                            str(i)] = raw_quarter.groupby(["permno"])["rsup"].shift(i)
            ...
            df_temp = raw_quarter.groupby(["fyear", "sic2"], as_index=False)[
                ["roavol", "sgrvol"]].median()
            df_temp = df_temp.rename(
                columns={"roavol": "md_roavol", "sgrvol": "md_sgrvol"})
            raw_quarter = pd.merge(raw_quarter, df_temp,
                                   how="left", on=["fyear", "sic2"])

            # Calculate quarterly values
            raw_quarter["m7"] = np.where(
                raw_quarter["roavol"] < raw_quarter["md_roavol"], 1, 0)
            raw_quarter["m8"] = np.where(
                raw_quarter["sgrvol"] < raw_quarter["md_sgrvol"], 1, 0)

            # Calculate final value
            raw_quarter["ms"] = raw_quarter["m1"] + raw_quarter["m2"] + raw_quarter["m3"] + \
                raw_quarter["m4"] + raw_quarter["m5"] + \
                raw_quarter["m6"] + raw_quarter["m7"] + raw_quarter["m8"]
            ...
            ccm3 = ccm2.copy()
            # we only consider the condition that the day after rdq is not a trading day, which is up to 5 days
            for i in range(6):
            ...
            combined_ccm3_crsp_d = pd.merge(
                crsp_d, ccm3, how="left", on=["permno", "rdq_trad"])

            combined_ccm3_crsp_d['ret_l1'] = combined_ccm3_crsp_d['ret'].shift(
                1)
            combined_ccm3_crsp_d['ret_l-1'] = combined_ccm3_crsp_d['ret'].shift(
                -1)
            combined_ccm3_crsp_d['ear'] = combined_ccm3_crsp_d['ret_l-1'] + \
                combined_ccm3_crsp_d['ret'] + combined_ccm3_crsp_d['ret_l1']

            combined_ccm3_crsp_d['vol_l1'] = combined_ccm3_crsp_d['vol'].shift(
                1)
            combined_ccm3_crsp_d['vol_l-1'] = combined_ccm3_crsp_d['vol'].shift(
                -1)
            combined_ccm3_crsp_d['vol_l10'] = combined_ccm3_crsp_d['vol'].shift(
                10)
            combined_ccm3_crsp_d['vol_1mth_average'] = combined_ccm3_crsp_d['vol_l10'].rolling(
                20).mean()
            combined_ccm3_crsp_d['3d_vol_avg'] = (
                combined_ccm3_crsp_d['vol_l-1'] + combined_ccm3_crsp_d['vol'] + combined_ccm3_crsp_d['vol_l1']) / 3
            combined_ccm3_crsp_d['aeavol'] = (
                combined_ccm3_crsp_d['3d_vol_avg'] - combined_ccm3_crsp_d['vol_1mth_average']) / combined_ccm3_crsp_d['vol_1mth_average']

            combined_ccm3_crsp_d = combined_ccm3_crsp_d.dropna(subset=['rdq'])
            df_ear_aeavol = combined_ccm3_crsp_d[[
                'date', 'datadate', 'rdq_trad', 'rdq', 'permno', 'ret', 'vol', 'ear', 'aeavol']]
            df_ear_aeavol['permno'] = df_ear_aeavol['permno'].astype('int')
            df_ear_aeavol.sort_values(["permno", "datadate"])

            raw_quarter = pd.merge(raw_quarter, df_ear_aeavol, how="left", on=[
                                   "permno", "datadate"])

            raw_quarter = raw_quarter.drop_duplicates(
                subset=['permno', 'datadate'])
            ...
            raw_month = pd.merge(crsp_msf, crsp_dlret, how="left", on=[
                                 "permno", "date_std"])
            ...
            raw_month["total_ret"] = (
                1 + raw_month["ret"]) * (1 + raw_month["dlret"]) - 1
            ...
            raw_month = raw_month.merge(
                comp_crsp_link[["permno", "linkdt", "linkenddt"]], how="left", on=["permno"])

            raw_month = raw_month[(raw_month['date'] >= raw_month['linkdt']) & (
                raw_month['date'] <= raw_month['linkenddt'])]
            raw_month = raw_month.drop(['linkdt', 'linkenddt'], axis=1)

            # Calculate lagged returns
            for i in range(36):
                raw_month["ret_l" +
                          str(i+1)] = raw_month.groupby(["permno"])["ret"].shift(i+1)
            ...
            raw_month["turn"] = raw_month[["vol_l1", "vol_l2", "vol_l3"]].mean(
                axis=1) / raw_month["shrout"]

            # dolvol
            raw_month["prc_l2"] = raw_month.groupby(["permno"])["prc"].shift(2)
            raw_month["dolvol"] = np.log(
                raw_month["vol_l2"] * raw_month["prc_l2"]).replace([np.inf, -np.inf], np.nan)
            ...
            raw_month = pd.merge(
                raw_month, sic2_link[["permno", "sic2"]], how="left", on=["permno"])
            df_temp = raw_month.groupby(["date_std", "sic2"], as_index=False)[
                "mom12m"].mean()
            df_temp = df_temp.rename(columns={"mom12m": "indmom"})
            raw_month = pd.merge(raw_month, df_temp, how="left", on=[
                                 "date_std", "sic2"])
            ...
            # daily
            if count != len(years):
                #self.parm = {"start_date": self.start_year_daily_date, "end_date": '12/31/'+str(year)}
                self.parm = {"start_date": '01/01/' +
                             str(year-5), "end_date": '12/31/'+str(year)}
            else:
                #self.parm = {"start_date": self.start_year_daily_date, "end_date": self.wrds_end_date}
                self.parm = {"start_date": '01/01/' +
                             str(year-5), "end_date": self.wrds_end_date}
            ...
            # baspread
            crsp_dsf1["baspread"] = (crsp_dsf1["askhi"] - crsp_dsf1["bidlo"]) / \
                ((crsp_dsf1["askhi"] + crsp_dsf1["bidlo"]) / 2)

            # ill
            crsp_dsf1["ill"] = abs(crsp_dsf1["ret"]) / \
                (abs(crsp_dsf1["prc"]) * crsp_dsf1["vol"])

            daily_features = crsp_dsf1.groupby(
                ["permno", "yyyy-mm"], as_index=False)[["baspread", "ill"]].mean()

            # maxret
            temp_maxret = crsp_dsf1.groupby(
                ["permno", "yyyy-mm"], as_index=False)["ret"].max()
            temp_maxret = temp_maxret.rename(columns={"ret": "maxret"})
            daily_features = pd.merge(
                daily_features, temp_maxret, how="left", on=["permno", "yyyy-mm"])

            # std_dolvol
            crsp_dsf1["std_dolvol"] = np.log(
                abs(crsp_dsf1["prc"] * crsp_dsf1["vol"])).replace([np.inf, -np.inf], np.nan)

            # std_turn
            crsp_dsf1["std_turn"] = crsp_dsf1["vol"] / crsp_dsf1["shrout"]

            # retvol
            temp_std = crsp_dsf1.groupby(
                ["permno", "yyyy-mm"], as_index=False)[["ret", "std_dolvol", "std_turn"]].std()
            temp_std = temp_std.rename(columns={"ret": "retvol"})
            daily_features = pd.merge(
                daily_features, temp_std, how="left", on=["permno", "yyyy-mm"])

            # zerotrade
            crsp_dsf1["countzero"] = np.where(crsp_dsf1["vol"] == 0, 1, 0)
            crsp_dsf1["ndays"] = 1
            crsp_dsf1["turn_alt"] = crsp_dsf1["vol"] / crsp_dsf1["shrout"]
            temp_zerotrade = crsp_dsf1.groupby(
                ["permno", "yyyy-mm"], as_index=False)[["countzero", "ndays", "turn_alt"]].sum()
            temp_zerotrade["zerotrade"] = (temp_zerotrade["countzero"] + (
                (1 / temp_zerotrade["turn_alt"]) / 480000)) * 21 / temp_zerotrade["ndays"]
            daily_features = pd.merge(daily_features, temp_zerotrade[[
                                      "permno", "yyyy-mm", "zerotrade"]], how="left", on=["permno", "yyyy-mm"])
            ...
            crsp_dsf = pd.merge(crsp_dsf, dlret, how='left',
                                on=['permno', 'date'])

            # return adjusted for delisting
            crsp_dsf['retadj'] = np.where(crsp_dsf['dlret'].notna(
            ), (crsp_dsf['ret'] + 1)*(crsp_dsf['dlret'] + 1) - 1, crsp_dsf['ret'])
            ...
            crsp_dsf = crsp_dsf[['date', 'permno',
                                 'ret', 'retadj', 'sprtrn', 'abrd']]

            crsp_dsf['weekly_moving_abrd_sum'] = crsp_dsf['abrd'].rolling(
                window=5).sum()
            crsp_dsf['weekly_moving_abrd_std'] = crsp_dsf['weekly_moving_abrd_sum'].rolling(
                156).std()

            crsp_dsf["yyyy-mm"] = crsp_dsf["date"].dt.to_period('M')

            df_idiovol = crsp_dsf.groupby(
                ["permno", "yyyy-mm"], as_index=False)[["weekly_moving_abrd_std"]].mean()
            df_idiovol.rename(
                columns={"weekly_moving_abrd_std": "idiovol"}, inplace=True)

            daily_features = pd.merge(
                daily_features, df_idiovol, how="left", on=["permno", "yyyy-mm"])
            daily_features = daily_features.sort_values(
                by=['permno', 'yyyy-mm'])
            ...
            parm = {"start_date": self.wrds_start_date,
                    "end_date": self.wrds_end_date}
            ...
            crsp_stocknames["permno"] = crsp_stocknames["permno"].astype(
                "Int64")
            month = pd.merge(
                month, crsp_stocknames[["ticker", "permno"]], how="left", on=["permno"])
            ...
            quarter["date_std"] = quarter["date_std"] + \
                MonthEnd(quarterly_lag + 1)
            ...
            combined = pd.merge(month, daily, how="left",
                                on=["permno", "date_std"])
            combined = combined.drop(["yyyy-mm"], axis=1)

            # Merge quarterly files
            combined = pd.merge(combined, quarter, how="left", on=[
                                "permno", "date_std"])
            combined = combined.drop(["datadate"], axis=1)

            # Merge annual files
            combined = pd.merge(combined, annual, how="left",
                                on=["permno", "date_std"])
            combined = combined.drop(["datadate"], axis=1)
            ...
            quarter_cols = ["cash", "chtx", "roaq", "roeq", "rsup",
                            "cinvest", "nincr", "roavol", "stdacc", "stdcf", "ms"]
            combined[quarter_cols] = combined[quarter_cols].ffill(
                axis=0, limit=3)
            ...
            combined[annual_cols] = combined[annual_cols].ffill(
                axis=0, limit=12)
            ...
            if count == 1:
                self.features_df = sp500_subset
            else:
                self.features_df = pd.concat(
                    [self.features_df, sp500_subset], ignore_index=True)
            count += 1

        # self.features_df.to_csv('features_df.csv')
            #sp500_subset.to_csv("../result/paper1/paper1_features.csv", index=False)
        #self.features_df.to_csv("paper1_features.csv", index=False)
        # return self.features_df

        #df = pd.read_csv('paper1_features_author.csv', index_col=0)
        df = self.features_df
        ...
        for k, v in dic.items():
            if v.shape[0] > 1:
                ...
                csv_file = v[(v['date'] >= pd.to_datetime(self.fred_start_date))
                             & (v['date'] <= pd.to_datetime(self.fred_end_date))]
                csv_file.set_index('date', inplace=True)
                csv_file.index = csv_file.index.to_period('M')
                ...
                if count == 1:
                    paper1_final = csv_file
                else:
                    try:
                        paper1_final = pd.concat(
                            [paper1_final, csv_file], ignore_index=True)
                    except:
                        # delisted companies
                        pass
                ticker_files[dic_map[k]] = csv_file
                print("\rPaper1 completed {}/{}.".format(count, 828),
                      end='')  # total 813 existing stocks
                sys.stdout.flush()
                count += 1
        paper1_final.to_csv('../result/sp500/paper1/paper1_features.csv')
        return ticker_files
...
if __name__ == "__main__":
    ...
    print('Please check /result/' + Paper1().stock_pool +
          '/paper1/paper1_features.csv')
