...
class Paper2:
    ...
    def sp500_generate(self):
        ...
        raw_quarterly = pd.read_csv(
            'paper2/raw_quarterly.csv', index_col='date')
        ...
        sp500_m = sp500_d.loc[sp500_d.groupby(
            sp500_d.index.to_period('M')).apply(lambda x: x.index.max())]
        ...
        djia_d = djia_d[['Close']]
        djia_m = djia_d.loc[djia_d.groupby(
            djia_d.index.to_period('M')).apply(lambda x: x.index.max())]
        ...
        def clean_datasets(dataset):
            ...
            dataset = dataset[dataset['date'].dt.year >=
                              1955][['date', 'value']]
        ...
        wrds_nyse = conn.raw_sql(
            """select caldt, totval, vwretx from crsp_q_indexes.msia where caldt >= '2000-01-01'""")
        wrds_sp500 = conn.raw_sql(
            """select caldt, vwretd, vwretx from crsp_q_indexes.msp500 where caldt >= '2000-01-01'""")
        ...
        monthly = pd.merge(
            monthly, raw_monthly[['yyyy-mm', 'ltr', 'corpr']], on=['yyyy-mm'], how='left')
        ...
        monthly = pd.merge(
            monthly, lty_combined[['yyyy-mm', 'lty']], on=['yyyy-mm'], how='left')
        ...
        monthly = pd.merge(
            monthly, tbl[['yyyy-mm', 'tbl', 'Rfree']], on=['yyyy-mm'], how='left')
        ...
        monthly = pd.merge(
            monthly, cpi[['yyyy-mm', 'infl']], on=['yyyy-mm'], how='left')
        ...
        sp500_d['svar'] = (sp500_d['Close'] /
                           sp500_d['Close'].shift(1) - 1) ** 2
        svar_calc = sp500_d[['yyyy-mm', 'svar']
                            ].groupby(['yyyy-mm']).sum().reset_index()
        ...
        wrds_sp500.rename(
            columns={'vwretd': 'CRSP_SPvw', 'vwretx': 'CRSP_SPvwx'}, inplace=True)
        monthly = pd.merge(monthly, wrds_sp500[[
                           'yyyy-mm', 'CRSP_SPvw', 'CRSP_SPvwx']], on=['yyyy-mm'], how='left')
        ...
        wrds_nyse['net_issue'] = wrds_nyse['totval'] * 1000 - \
            wrds_nyse['totval'].shift(1) * 1000 * (1 + wrds_nyse['vwretx'])
        wrds_nyse['net_issue_12'] = wrds_nyse['net_issue'].rolling(
            min_periods=12, window=12).sum()

        wrds_nyse['ntis'] = wrds_nyse['net_issue_12'] / \
            (wrds_nyse['totval'] * 1000)
        monthly = pd.merge(
            monthly, wrds_nyse[['yyyy-mm', 'ntis']], on=['yyyy-mm'], how='left')

        # Calculate e12 and d12 based on interpolating quarterly data

        raw_quarterly['E12'] = raw_quarterly['eps'].rolling(
            min_periods=4, window=4).sum()
        raw_quarterly['D12'] = raw_quarterly['dps'].rolling(
            min_periods=4, window=4).sum()
        raw_quarterly.index = pd.to_datetime(
            raw_quarterly.index, dayfirst=True)
        raw_quarterly = raw_quarterly.reindex(pd.date_range(
            start=raw_quarterly.index.min(), end=raw_quarterly.index.max(), freq='M'))
        raw_quarterly = raw_quarterly.interpolate(
            method='linear', limit_area='inside')

        raw_quarterly['yyyy-mm'] = raw_quarterly.index.to_period('M')
        monthly = pd.merge(
            monthly, raw_quarterly[['yyyy-mm', 'E12', 'D12']], on=['yyyy-mm'], how='left')
        ...
        calc_bm['book_year'] = calc_bm['yyyy-mm'].apply(
            lambda x: x.year - 1 if x.month > 2 else x.year - 2)
        calc_bm['book'] = calc_bm['book_year'].map(raw_annual['djia_book'])
        calc_bm['b/m'] = calc_bm['book'] / calc_bm['djia']

        monthly = pd.merge(
            monthly, calc_bm[['yyyy-mm', 'b/m']], on=['yyyy-mm'], how='left')
        ...
        deflator = deflator[deflator['date'].dt.year >=
                            1947][['date', 'value']]
        ...
        # Calculate i/k
        allI = pnfir / 4
        # Depreciation rate for making capital out of investment - follows Goyal & Welch (2008)
        delta = 0.1 / 4
        # Stationary i/k in k_t+1 = (1-delta)k_t + i_t - follows Goyal & Welch (2008)
        ikbar = delta + mean(allI[1:] / allI[:-1]) - 1
        allk = 0 * allI
        allk[0] = allI[0] / ikbar
        ...
        monthly = pd.merge(
            monthly, ik[['yyyy-mm', 'ik']], on=['yyyy-mm'], how='left')
        monthly['ik'] = monthly['ik'].ffill(axis=0, limit=3)

        # Calculate eqis
        monthly['year'] = monthly['date'].dt.year
        raw_annual['year'] = raw_annual.index
        eqis = raw_annual[raw_annual['year'] >=
                          1955][['year', 'equity', 'debt']]
        ...
        monthly = pd.merge(monthly, eqis[['year', 'eqis']], on=[
                           'year'], how='left')
        ...
        features.to_csv(
            "../result/sp500/paper2/paper2_features.csv", index=False)
...
if __name__ == "__main__":
    ...
    print('Please check /result/' + Paper2().stock_pool +
          '/paper2/paper2_features.csv')
