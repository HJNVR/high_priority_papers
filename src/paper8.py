...
# Feature 7: cashdebt
raw_annual["cashdebt"] = (raw_annual["ib"] + raw_annual["dp"]) / \
    ((raw_annual["lt"] + raw_annual["lt_l1"]) / 2)
...
# Feature 25: grcapx
raw_annual['grcapx'] = (raw_annual['capx'] -
                        raw_annual['capx_l2']) / raw_annual['capx_l2']
...
raw_annual['grltnoa'] = ((raw_annual['rect']+raw_annual['invt']+raw_annual['ppent']+raw_annual['aco']+raw_annual['intan'] +
                         ...
                         - (raw_annual['rect_l1']+raw_annual['invt_l1']+raw_annual['ppent_l1']+raw_annual['aco_l1']
                            ...
                            raw_annual['ap_l1']-raw_annual['lco_l1']
                            - raw_annual['lo_l1'])
...
raw_annual = pd.merge(raw_annual, df_temp, how='left', on=['datadate', 'sic2'])
raw_annual['herf'] = (raw_annual['sale']/raw_annual['indsale']) * \
    (raw_annual['sale']/raw_annual['indsale'])
df_temp = raw_annual.groupby(['datadate', 'sic2'], as_index=False)[
    'herf'].sum()
...
raw_annual["pchcapx"] = (raw_annual["capx"] -
                         raw_annual["capx_l1"]) / raw_annual["capx_l1"]
...
# Feature 37: pchdepr
raw_annual["pchdepr"] = ((raw_annual["dp"] / raw_annual["ppent"]) - (raw_annual["dp_l1"] /
                         raw_annual["ppent_l1"])) / (raw_annual["dp_l1"] / raw_annual["ppent_l1"])
...
choicelist = ((((raw_annual["txt"] - raw_annual["txdi"]
                 ) / raw_annual["tr"]) / raw_annual["ib"]), 1)
...
raw_quarter["chtx"] = (raw_quarter["txtq"] -
                       raw_quarter["txtq_l4"]) / raw_quarter['atq_l4']
...
raw_quarter["rsup"] = (raw_quarter["saleq"] -
                       raw_quarter["saleq_l4"]) / raw_quarter["mveq"]
...
raw_quarter["cinv_1"] = (raw_quarter["ppentq_l1"] -
                         raw_quarter["ppentq_l2"]) / raw_quarter["saleq_l1"]
raw_quarter["cinv_2"] = (raw_quarter["ppentq_l2"] -
                         raw_quarter["ppentq_l3"]) / raw_quarter["saleq_l2"]
raw_quarter["cinv_3"] = (raw_quarter["ppentq_l3"] -
                         raw_quarter["ppentq_l4"]) / raw_quarter["saleq_l3"]
...
raw_quarter["cinv_4"] = (raw_quarter["ppentq_l1"] -
                         raw_quarter["ppentq_l2"]) / 0.01
raw_quarter["cinv_5"] = (raw_quarter["ppentq_l2"] -
                         raw_quarter["ppentq_l3"]) / 0.01
raw_quarter["cinv_6"] = (raw_quarter["ppentq_l3"] -
                         raw_quarter["ppentq_l4"]) / 0.01
...
for i in range(8):
    ...
    if i == 0:
        raw_quarter["nincr_t" + str(i+1)] = np.where(raw_quarter["ibq"]
                                                     > raw_quarter["ibq_l1"], 1, 0)
    else:
...
comp_annual1 = pd.merge(comp_annual1, df_temp,
...
combined_ccm3_crsp_d['aeavol'] = (combined_ccm3_crsp_d['3d_vol_avg'] -
                                  combined_ccm3_crsp_d['vol_1mth_average']) / combined_ccm3_crsp_d['vol_1mth_average']
...
raw_quarter = pd.merge(raw_quarter, df_ear_aeavol,
                       how="left", on=["permno", "datadate"])
...
raw_month = pd.merge(crsp_msf, crsp_dlret, how="left",
                     on=["permno", "date_std"])
...
daily_features = pd.merge(daily_features, temp_maxret,
                          how="left", on=["permno", "yyyy-mm"])
...
daily_features = pd.merge(daily_features, temp_std,
                          how="left", on=["permno", "yyyy-mm"])
...
daily_features = pd.merge(daily_features, df_idiovol,
                          how="left", on=["permno", "yyyy-mm"])
...
parm_sp500 = {
    "start_year_sp500_companies": start_year_sp500_companies, "end_year": end_year}
