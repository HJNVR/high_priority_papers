...
# Execution complete in 19:21:28.15
# ============================================================================
...
class Paper1:
  ...
  # Load config file
  def generate(self):
    years = list(range(int(self.wrds_start_date[-4:]), int(self.wrds_end_date[-4:]) + 1))
    count = 1
    for year in years:
      ...
      if count != len(years):
        self.parm = {"start_date": '01/01/'+str(year), "end_date": '12/31/'+str(year)}
      else:
        self.parm = {"start_date": '01/01/'+str(year), "end_date": self.wrds_end_date}
      ...
      cpi_data = pd.DataFrame({'DATE':cpi_data.index, 'CPIAUCSL':cpi_data.values})
      ...
      if count != len(years):
        ...
        self.parm = {"start_date": '01/01/'+str(year-5), "end_date": '12/31/'+str(year)}
      else:
        #self.parm = {"start_date": self.start_year_daily_date, "end_date": self.wrds_end_date}
        self.parm = {"start_date": '01/01/'+str(year-5), "end_date": self.wrds_end_date}
      ...
      annual_lag = self.data['annual_lag']
      quarterly_lag = self.data['quarterly_lag']
      #sp500_companies_subset = data['S&P500_companies_only']

      parm = {"start_date": self.wrds_start_date, "end_date": self.wrds_end_date}
      ...
      if count == 1:
        ...
        self.features_df = pd.concat([self.features_df, sp500_subset], ignore_index=True)
      count += 1
    ...
    for k, v in dic.items():
      if v.shape[0] > 1:
        ...
        if count == 1:
          ...
          try:
            paper1_final = pd.concat([paper1_final, csv_file], ignore_index=True)
          except:
            # delisted companies
            pass
        ticker_files[dic_map[k]] = csv_file
        print("\rPaper1 completed {}/{}.".format(count, 828), end='') # total 813 existing stocks
        sys.stdout.flush()
        count += 1
    paper1_final.to_csv('../result/paper1/paper1_features.csv')
    return ticker_files
